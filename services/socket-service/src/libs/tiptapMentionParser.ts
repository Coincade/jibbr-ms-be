 ...
 */
export function parseMentionsFromJSON(
  json: any
  ...
  if (json.content && Array.isArray(json.content)) {
    ...
    walkNode(json as ProseMirrorNode);
  }
 ...
 */
export function parseMentionsFromHTML(
  html: string
): Array<{ userId: string; username: string }> {
  ...
  if (mentions.size === 0) {
    ...
    while ((plainMatch = plainMentionRegex.exec(html)) !== null) {
      ...
      // Note: This won't have userId; resolving via username would need extra lookup.
    }
  }
