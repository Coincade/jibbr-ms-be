...
router.get(
  ...
  getOnlineUsersList as unknown as RequestHandler
);
...
router.get(
  '/online/:userId',
  authMiddleware(process.env.JWT_SECRET!) as unknown as RequestHandler,
  checkUserOnlineStatus as unknown as RequestHandler
);

// Check online status for multiple users
router.post(
  '/online/check-multiple',
  authMiddleware(process.env.JWT_SECRET!) as unknown as RequestHandler,
  checkMultipleUsersStatus as unknown as RequestHandler
);
...
router.get(
  '/stats',
  authMiddleware(process.env.JWT_SECRET!) as unknown as RequestHandler,
  getOnlineStats as unknown as RequestHandler
);
